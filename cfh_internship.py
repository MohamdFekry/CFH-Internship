...
# the high and low prices could be dropped from the training data because of their high correlation with the open feature
...
"""# Predicting a day in the future"""

print(df['time'].iloc[-1])
print(df['close'].iloc[-1])

# the last date of the data is 2024-02-15
future = df.iloc[-1]
future['day'] = 16
future['open'] = df['close'].iloc[-1]

# the high and low prices for the prediction sample below were set equal to the open price
# because of the high correlation between them
future_pred = model_1.predict(np.array([future[['day', 'month', 'year', 'open', 'open', 'open']],]))
print(future_pred)
